...
public class Instance4 {
   ...
   // private static final Logger log = LoggerFactory.getLogger(Instance.class);
    ...
    public Instance4(String email, String password) {
        this.email = email;
        this.password = password;
    }

    public Instance4(String token) {
        this.token = token;
    }
  ...
  //                 log.warn("Failed to reconnect bot", e);
    ...
    public void onMessage(MessageReceivedEvent e) {
    	...
    	String s = message;
    	try {    		       
          if(e.getMessage().getChannel().getID().contains("263473469931520002") || e.getMessage().getAuthor().getID().contains("263470411839307787")){
